...
{
    internal class ScrollingTextPanel
    {
        SpriteFont _font;
        string _text;
        ...
        private string parseText(string text)
        {
            ...
            _textboxPosition.Y += _font.MeasureString(line).Y;
            for (int i = 0; i < wordArray.Length; i++)
            {
                if (_font.MeasureString(line).Y + _textboxPosition.Y < _textboxPosition.Y + _textboxRectangle.Height
                   || _textboxPosition.Y > _textboxRectangle.Y)
                {
                    continue;
                }
            }
        ...
        {
            spriteBatch.DrawString(Globals.DefaultFont, _text, _textboxPosition, Color.White);
        }
    }
}
